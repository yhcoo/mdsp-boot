package com.yhcoo.auth.controller;

import com.yhcoo.auth.query.OAuth2AccessTokenQuery;
...
public class AuthController {
    ...
    public ApiResult getTokenList(OAuth2AccessTokenQuery oAuth2AccessTokenQuery) {
        ...
        oAuth2AccessTokenQuery.setRecords((List)redisTemplate.opsForList().range(CommonConstants.MDSP_REDIS_LIST_LEY, oAuth2AccessTokenQuery.getStart(), oAuth2AccessTokenQuery.getEnd()));
        List<OAuth2AccessToken> all = (List)redisTemplate.opsForList().range(CommonConstants.MDSP_REDIS_LIST_LEY, 0, -1);
        if(all != null){
